...
class DeepPlotAgent:
    ...
    INITIAL_PROMPT_WITH_FILES = """Analyze the file(s): {filenames}. You have {timeout_seconds} seconds.

**Research process:**
1. **Look** - Load the data. What is this dataset about?
2. **Ask** - What questions are worth investigating? State them explicitly.
...
{user_prompt}"""
    ...
    INITIAL_PROMPT_NO_FILES = """Explore the working directory and find data files to analyze. You have {timeout_seconds} seconds.
...
2. Write your complete markdown report to `analysis.md` using the Write tool.
3. Call `submit_summary(evidence_plots=[...])` with the plot IDs that support your findings.
"""
    ...
    async def run(self) -> dict:
        ...
        try:
            ...
            # Initial query - select prompt based on whether files provided
            self.iteration_count = 1
            if self.data_files:
                filenames = ', '.join(f'`{f}`' for f in self.data_files)
                initial_prompt = self.INITIAL_PROMPT_WITH_FILES.format(
                    filenames=filenames,
                    user_prompt=self.user_prompt if self.user_prompt else "",
                    timeout_seconds=self.timeout_seconds
                )
            else:
                initial_prompt = self.INITIAL_PROMPT_NO_FILES.format(
                    user_prompt=self.user_prompt if self.user_prompt else "",
                    timeout_seconds=self.timeout_seconds
                )
