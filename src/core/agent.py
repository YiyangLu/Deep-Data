        ...
        try:
            async for msg in self._client.receive_response():
                ...
                if msg_type == 'SystemMessage':
                    ...
                    if 'cwd' in system_data and self.cwd:
                        # self.cwd is always absolute (resolved in __post_init__)
                        if system_data['cwd'] != self.cwd:
                            raise ValueError(
                                f"CWD mismatch: expected {self.cwd}, got {system_data['cwd']}"
                            )
