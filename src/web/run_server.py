...
Usage:
    ...
    python -m src.web.run_server --host 0.0.0.0 --port 8000
    python -m src.web.run_server --cwd /path/to/project

    # MLE CLI mode (no browser needed)
    python -m src.web.run_server --mle --preset mlebench --cwd /path/to/project
    python -m src.web.run_server --mle --auto --cwd /path/to/project
"""
...
Examples:
  # Web UI mode
  python -m src.web.run_server
  python -m src.web.run_server --cwd ./my-project

  # MLE CLI mode
  python -m src.web.run_server --mle --preset mlebench --cwd ./my-project
  python -m src.web.run_server --mle --auto --cwd ./my-project
  python -m src.web.run_server --mle --preset mlebench -t 3600 -n 3 -m opus
        """
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="[MLE] Number of parallel workers (default: 3)"
    )
    parser.add_argument(
        "--max-steps",
        type=int,
    ...
    print("╔════════════════════════════════════════════════════════════╗")
    print("║       Unified Agent Server - All-in-One                   ║")
    print("╚════════════════════════════════════════════════════════════╝")
    print()
    print(f"🚀 Server: http://{args.host}:{args.port}")
    print(f"   • Chat UI: http://{args.host}:{args.port}/")
    print(f"   • WebSocket: ws://{args.host}:{args.port}/ws")
    print(f"   • Plots: http://{args.host}:{args.port}/plot/{{id}}")
    ...
    if args.model:
        ...
        print(f"🤖 Model: (default from ~/.claude/settings.json)")

    # Display resume mode if active
    if args.resume:
        print(f"🔄 Resume mode: Resuming session '{args.resume}'")
    elif args.continue_last:
        print(f"🔄 Resume mode: Continuing last session for this directory")
    else:
        print(f"✨ New session mode")

    print()
    print("✨ Features:")
    print("   • Agent conversation with streaming")
    print("   • Interactive Plotly visualizations")
    print("   • Plot interaction capture & logging")
    print("   • Session storage to ./logs/sessions/")
    print()
    print("💡 Frontend development:")
    print("   cd src/web/frontend && npm run dev")
    print()
