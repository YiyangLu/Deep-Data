 ...
 */
export function InputBox({ onSend, onDeepPlot, disabled, isConnected, isProcessing, sessionId, plotTabs, onRecoverPlot, stats, sessionType }) {
  const [input, setInput] = useState('');
  const [copied, setCopied] = useState(false);
  const [uploadedFiles, setUploadedFiles] = useState([]); // Array of { name, uploading, error }
  const [deepPlotDuration, setDeepPlotDuration] = useState(120); // Default 2 minutes
  ...
  return (
    <div className="p-4 pb-6">
      <div className="max-w-3xl mx-auto">
        {/* Status bar above input: left = plot history, right = token stats */}
        <div className="flex items-center justify-between mb-2 px-1">
          <PlotHistoryDropdown sessionId={sessionId} plotTabs={plotTabs} onRecoverPlot={onRecoverPlot} />

          {/* Token stats - clickable to copy */}
          {stats && (
            <button
              onClick={handleCopyStats}
              className="flex items-center gap-3 text-xs text-gray-500 hover:text-gray-700 cursor-pointer transition-colors"
            ...
            <div className="flex items-center gap-1">
              ...
              {(sessionType === 'deep_plot' || uploadedFiles.length > 0) && (
                <div className="flex items-center gap-1 px-2 py-1 bg-purple-100 rounded-full">
                  ...
                  <select
                    value={deepPlotDuration}
                    onChange={(e) => setDeepPlotDuration(Number(e.target.value))}
                    className="bg-transparent text-purple-700 text-sm font-medium cursor-pointer focus:outline-none"
                  >
