...
export function PlotPanel({
  ...
  const hasLoggedInit = useRef(false);  // Track if we've logged init for current plot
  const pendingCommand = useRef(null);  // Store command to execute after plot renders
  const pendingStateRestore = useRef(null);  // Store cumulative_state to restore after plot renders
  const isRestoringState = useRef(false);  // Flag to prevent logging during state restoration

  // Parse session_id and plot_id from plotUrl
  const parseUrl = useCallback((url) => {
    if (!url) return null;
    // URL format: /plot/{session_id}/{plot_id}
    const match = url.match(/\/plot\/([^/]+)\/(\d+)/);
    if (match) {
      return { sessionId: match[1], plotId: parseInt(match[2], 10) };
    }
    return null;
  }, []);

  // Fetch plot JSON when URL changes
  useEffect(() => {
    ...
    // Reset flags when plot changes
    hasLoggedInit.current = false;
    pendingStateRestore.current = null;
    setIsLoading(true);
    setError(null);

    fetch(`/plot/api/json/${parsed.sessionId}/${parsed.plotId}`)
      .then(response => {
        if (!response.ok) {
      ...
      .then(data => {
        // Store cumulative_state for restoration after plot renders
        if (data.cumulative_state) {
          pendingStateRestore.current = data.cumulative_state;
        }
        setPlotData(data);
        setIsLoading(false);
  ...
  const captureScreenshot = useCallback(async () => {
    if (!plotRef.current || !plotRef.current.el) return null;
    try {
  ...
  // Restore plot state from cumulative_state (without logging)
  ...
  // Handle plot rendered - capture screenshot on init, restore state, and execute pending commands
  const handleAfterPlot = useCallback(async () => {
    const parsed = parseUrl(plotUrl);
    if (!parsed) return;

    // Check if we're resuming (cumulative_state exists) vs fresh plot
    // If cumulative_state exists at all, this is a resumed session - don't log init
    const isResuming = pendingStateRestore.current !== null;

    // Check if there's meaningful state to actually restore (ranges or hidden traces)
    const hasMeaningfulState = pendingStateRestore.current && (
      pendingStateRestore.current['xaxis.range[0]'] !== undefined ||
      pendingStateRestore.current['yaxis.range[0]'] !== undefined ||
      pendingStateRestore.current['xaxis.range'] !== undefined ||
      pendingStateRestore.current['yaxis.range'] !== undefined ||
      (pendingStateRestore.current.trace_visibility &&
       Object.keys(pendingStateRestore.current.trace_visibility).length > 0)
    );

    // Log init event (only once per plot, and only if NOT resuming)
    if (!hasLoggedInit.current) {
      hasLoggedInit.current = true;

      if (isResuming) {
        // Resuming session - don't log init
        // Only restore if there's meaningful state (zoom or hidden traces)
        if (hasMeaningfulState) {
          setTimeout(() => {
            restoreState(pendingStateRestore.current);
            pendingStateRestore.current = null;
          }, 300);
        }
      } else {
        // Fresh plot - log init
        setTimeout(async () => {
          logEvent(parsed.sessionId, parsed.plotId, 'init', { plot_id: parsed.plotId });
        }, 500);
      }
    }
    ...
    if (pendingCommand.current) {
      ...
      // Check if the pending command is for this plot
      if (cmd.sessionId === parsed.sessionId && cmd.plotId === parsed.plotId) {
        setTimeout(() => {
          executeCommand(cmd, parsed);
        }, 200);
      }
      pendingCommand.current = null;
    }
  }, [plotUrl, parseUrl, logEvent, executeCommand, restoreState]);
