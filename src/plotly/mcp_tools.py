...
@tool(
    "show_plot",
    "Create Plotly figure from code. Returns {plot_id}.",
    {
        "type": "object",
        "properties": {
            "plotly_codes": {
...
async def _mcp_show_plot(args: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    plot_id, _ = await show_plot(args["plotly_codes"])
    return {"plot_id": plot_id}


@tool(
    "query_interactions",
    "Get interaction history. Returns [{id, event_type, payload, has_screenshot}].",
    {
        "type": "object",
        "properties": {
            "plot_id": {
...
async def _mcp_query_interactions(args: dict[str, Any]) -> dict[str, Any]:
    ...
    Returns:
        Result dict:
        {
            "events": [
                {
                    "id": int,           # Use with get_plot_image(interaction_id=...)
...
async def _mcp_get_plot_image(args: dict[str, Any]) -> dict[str, Any]:
    ...
    Args:
        ...
        args["interaction_id"]: Optional specific interaction ID (from query_interactions)
...
def create_plotly_mcp_server(
    ...
    Example:
        >>> from src.plotly.mcp_tools import create_plotly_mcp_server
