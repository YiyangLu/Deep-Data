...
async def create_plot(request: Request):
    """
    Create a new plot and return its session-scoped ID.
    ...
    try:
        ...
        # Add plot to store (session-scoped)
        plot_id, _ = plot_store.add_plot(session_id, fig_json)

        return {
            'plot_id': plot_id,
            'session_id': session_id,
            'url': f'/plot/{session_id}/{plot_id}'
        }
...
async def get_plot_json(session_id: str, plot_id: int):
    ...
    cumulative_state for restoring previous view state.
    ...
    # Get cumulative state for state restoration on resume
    cumulative_state = None
    try:
        session_store = get_session_store(get_logs_root())
        cumulative_state = session_store.get_plot_cumulative_state(session_id, plot_id)
    except Exception as e:
        logger.warning(f"Failed to get cumulative state for plot {plot_id}: {e}")
